...
(ang. trade-offs) pomiędzy wielkością bloku wątków, intensywnością obliczeniową 
a fizycznymi ograniczeniami zasobów sprzętowych architektury Pascal 
(NVIDIA GeForce GTX 1080). Dobór rozmiaru instancji testowej ($3200\times3200$) okazał 
się w pełni uzasadniony - zapewnił ciągłe nasycenie multiprocesorów (SM) pracą, 
minimalizując błędy pomiarowe na styku wykonywania kernela. Wszystkie zaimplementowane 
warianty algorytmu pomyślnie przeszły testy walidacyjne z uwzględnieniem tolerancji 
błędu względnego dla obliczeń zmiennoprzecinkowych, co potwierdza ich poprawność.  
Głównym i najważniejszym wnioskiem płynącym z eksperymentu jest fakt, że maksymalizacja 
wskaźnika zajętości GPU (achieved occupancy) nie jest warunkiem koniecznym do osiągnięcia 
najwyższej wydajności obliczeniowej. Tezę tę doskonale ilustruje wariant 4-Res (BS=16). 
Pomimo spadku parametru occupancy do poziomu 74.7\% (wymuszonego wysokim zużyciem rejestrów 
na wątek i ograniczeniem liczby aktywnych bloków na SM z 8 do 6), wariant ten osiągnął najwyższą 
przepustowość obliczeniową wynoszącą 211.22 GFlop/s oraz najkrótszy czas wykonania (310.28 ms). 
Kluczem do sukcesu okazało się drastyczne podniesienie równoległości na poziomie instrukcji (ILP) 
oraz przeniesienie operacji z pamięci współdzielonej bezpośrednio na ultraszybki plik rejestrów. 
Spowodowało to wzrost wskaźnika IPC do rekordowej wartości 1.90 oraz zredukowało liczbę transakcji 
Global Load aż o 50% w porównaniu do bazowego algorytmu. 
\\ \\
Eksperyment ujawnił jednak sztywne \textbf{limity sprzętowe}, z którymi musi mierzyć się programista 
systemów równoległych:
\begin{itemize}
 \item Zwiększanie intensywności arytmetycznej przynosi wymierne korzyści, 
dopóki zasoby sprzętowe nie zostaną wyczerpane.
 \item Próba uruchomienia wariantu 4-Res dla bloku $32\times32$ zakończyła 
się niepowodzeniem. Mimo że zapotrzebowanie na pamięć współdzieloną (16 KB) 
mieściło się w limicie 48 KB, agresywne unrollowanie pętli oraz rozbudowany stan wewnętrzny 
jądra doprowadziły do przekroczenia alokacji rejestrów (maksymalnie 32 rejestry na wątek przy chęci 
zachowania minimalnej równoległości) i błędu zasobów hardware'owych.
\end{itemize}
