...
zdarzeń sprzętowych (hardware performance counters). Narzędzie mierzy m.in. liczbę wycofanych 
instrukcji (\textit{Instructions Retired}), liczbę taktów procesora (\textit{Clockticks}) 
oraz klasyfikuje przyczyny nieefektywności przetwarzania. Skorzystano również dla kodów 
równoległych z testu \textbf{Threading}, który pozwala na analizę efektywnego wykorzystania CPU oraz
podziału czasu pracy między sekcję równoległą a sekwencyjną, oraz testu \textbf{Hotspots}, który 
daje informacje o czasie przetwarzania CPU i jego podziale między czas efektywny a czasy oczekiwania.
W ramach analizy kodów k3 i k3a został też przeprowadzony test \textbf{Memory Access}, który pozwolił
...
W ramach tego eksperymentu porównywane za pomocą testów \textbf{Memory Access} i \textbf{Hotspots} zostały kody
k3 i k3a. Celem było zbadanie wpływu optymalizacji lokalności danych w kodzie k3a na czasy oczekiwania, oraz analiza
przyczyn tych oczekiwań. Skupiliśmy się na teście Bandwith, oraz rodzaju czasu oczekiwania Memory Bound dla poszczególnych kodów.
\begin{figure}[h]
    \centering
...
Poniżej przedstawiamy porównania wyników testu threading dla kodu równoległego k4.
